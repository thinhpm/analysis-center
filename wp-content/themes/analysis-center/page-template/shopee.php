...
<div class="container">
	...
	<div class="store-listings st-list-coupons" style="padding: 0 50px">
		<?php 
			$lists = $wpdb->get_results("SELECT * FROM voucher WHERE `website`='" . $website . "' AND `status` = 1 ORDER BY `voucher`.`id` DESC");

			$stt = 0;
			$listCount = count($lists);

			$listProduct = $wpdb->get_results("SELECT link_product FROM `products` where id_web= " . $idWeb . " ORDER BY `products`.`id` DESC LIMIT " . $listCount);

			foreach ($lists as $key=>$item) {
				$linkAff = isset($listProduct[$key]->link_product) ? $listProduct[$key]->link_product : 'https://' . $website . '.vn';
				$linkAff ="https://go.isclix.com/deep_link/4945784097639239041?url=" . $linkAff;
				$stt += 1;
				?>
					<div data-id="<?php echo $stt ?>" class="coupon-item store-listing-item has-thumb c-cat c-type-code shadow-box coupon-live">
						<div class="coupon-mgg-wrap">
							<div class="store-thumb-link">
				 			...
				 			<div class="coupon-detail coupon-button-type">
				 				<?php 
				 					if (true) {
				 						?>
					 						<a rel="nofollow" data-type="code" data-coupon-id="<?php echo $item->id ?>" href="javascript:void(0)" class="coupon-button coupon-code btn-get-code" data-tooltip="Nhấn để lấy mã" data-code="<?php echo $item->code ?>" data-aff-url="<?php echo $linkAff ?>">
				 		...
				 		<div class="clear"></div>
				 		<div id="myModal-<?php echo $stt ?>" class="modal fade modal-voucher" role="dialog" data-aff-url="<?php echo $linkAff ?>">
